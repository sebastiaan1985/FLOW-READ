...
export function ProgressBar({value,color=colors.accent,style}:{value:number;color?:string;style?:StyleProp<ViewStyle>}){return <View accessibilityRole="progressbar" accessibilityValue={{min:0,max:100,now:Math.round(value*100)}} style={[s.track,style]}><View style={{height:'100%',width:`${Math.max(0,Math.min(1,value))*100}%`,backgroundColor:color,borderRadius:99}}/></View>;}
export function Screen({children,style,scroll=true}:{children:ReactNode;style?:StyleProp<ViewStyle>;scroll?:boolean}){const inner=<View style={[s.screen,style]}>{children}</View>;return scroll?<ScrollView style={{flex:1}} contentContainerStyle={{flexGrow:1}} keyboardShouldPersistTaps="handled">{inner}</ScrollView>:inner;}
