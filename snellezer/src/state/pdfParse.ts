...
/* Tekst uit een PDF halen met pdf.js. Gedeeld door de webversie (pdfWeb.ts) en de app (PdfExtractor.tsx). */
...
export async function readPdf(pdfjs: any, bytes: Uint8Array, fileName: string, onProgress?: (page: number, pages: number) => void, cMapUrl?: string): Promise<ParsedBook> {
  let pdf: any;
  try {
    pdf = await pdfjs.getDocument({data: bytes, isEvalSupported: false, disableFontFace: true, ...(cMapUrl ? {cMapUrl, cMapPacked: true} : {})}).promise;
  } catch (e: any) {
