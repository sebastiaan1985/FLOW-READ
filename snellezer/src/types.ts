...
export type AppState = { profile: Profile; settings: ReadingSettings; sessions: SessionResult[]; texts: SavedText[]; targetWpm: number; kidsMode: boolean; baseline: { wpm: number; comprehension: number } | null; tempoStreak: number; reminder: { enabled: boolean; hour: number; minute: number }; leeslab: { lessons: Record<string, { best: number; mastered: boolean }>; review: string[] }; topTechniques: string[]; installHintDismissed: boolean; };
export type TrainingRequest = { exerciseId: string; daily?: boolean; baseline?: boolean; text?: SavedText };
export type AppActions = { onStart: (request: TrainingRequest) => void; onTab: (tab: TabId) => void; onLibrary: () => void; onSettings: () => void; onProfile: () => void; onPrivacy: () => void; onLab: (lessonId: string) => void; };
